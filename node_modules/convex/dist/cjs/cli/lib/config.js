...
async function parseProjectConfig(ctx, obj) {
  ...
  if (typeof obj.codegen.fileType !== "undefined" && obj.codegen.fileType !== "ts" && obj.codegen.fileType !== "js/dts") {
    return await ctx.crash({
      exitCode: 1,
      errorType: "invalid filesystem data",
      printedMessage: 'Expected `codegen.fileType` in `convex.json` to be "ts" or "js/dts"'
    });
  }
  if (obj.generateCommonJSApi && obj.codegen.fileType === "ts") {
    return await ctx.crash({
      exitCode: 1,
      errorType: "invalid filesystem data",
      printedMessage: 'Cannot use `generateCommonJSApi: true` with `codegen.fileType: "ts"`. CommonJS modules require JavaScript generation. Either set `codegen.fileType: "js/dts"` or remove `generateCommonJSApi`.'
    });
  }
  ...
  if (typeof obj.codegen.staticApi !== "boolean" || typeof obj.codegen.staticDataModel !== "boolean" || typeof obj.codegen.legacyComponentApi !== "undefined" && typeof obj.codegen.legacyComponentApi !== "boolean") {
    return await ctx.crash({
      exitCode: 1,
      errorType: "invalid filesystem data",
      printedMessage: "Expected `codegen.staticApi`, `codegen.staticDataModel`, and `codegen.legacyComponentApi` in `convex.json` to be booleans"
    });
  }
  return obj;
}
...
async function handlePushConfigError(ctx, error, defaultMessage, deploymentName, deployment) {
  ...
  if (data?.code === "RaceDetected") {
    ...
    return await ctx.crash({
      ...
      errorType: "transient",
      errForSentry: error,
      printedMessage: import_chalk.default.yellow(message)
    });
  }
